	    ...
	    openProcessingDialog:function(processingMessage, closable, modal) {
			...
			if(!$("#processingDialog") || $("#processingDialog").length <= 0) {
				var html = '<div id="processingDialog" modal="'+modal+'" closable="'+closable+'" icon="icon-save" style="overflow-y:hidden;padding:5px;width:350px;height:100px;text-align:center">'
								+'<div id="processingDiv"><table><tr><td><img src="../../resource/img/loading.gif"/></td><td><font color="green" id="processingMessage"></font></td></tr></table</div>'
						+'</div>';
				$("body").append(html);
			}
...
/*       error: function(XMLHttpRequest, textStatus, errorThrown) {
       ...
       beforeSend: function(XMLHttpRequest) {
    	   if(this.url.indexOf("?")==-1){
        	   this.url += "?ssid=" + getCookie("JSESSIONID");
    	   }else{
        	   this.url += "&ssid=" + getCookie("JSESSIONID");
    	   }
       },
       complete: function(req, status) {
    	   if(req.readyState==4 && req.status==200){
    		   try{
    	           ...
    	           if(reqObj && (reqObj.sessionStatus == "timeout")){
    	        	   window.top.$.messager.alert('提示消息', '会话过期', 'warning', function(){
    	        		   forward2Login();
    	        	   });	
    	           }
    	       }catch(e){
